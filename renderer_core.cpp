                               ...
                               std::atomic<uint32_t>                              *progressIndex)
{
	while (running->load(std::memory_order_relaxed))
	{
		...
		const bool debuggerAttached =
		    ...
		    false;
#endif
		int64_t allowedSeconds = (suppressed && suppressed->load(std::memory_order_relaxed)) ? 60 : 10;
		if (debuggerAttached && allowedSeconds < 120)
		{
			allowedSeconds = 120;
		}
		...
		if (elapsed >= allowedSeconds)
		{
			...
			if (debuggerAttached)
			{
				std::cerr << "Debugger attached; watchdog will not abort.\n";
				std::cerr << "========================================\n\n";
				lastFrameTime->store(std::chrono::steady_clock::now(), std::memory_order_relaxed);
				continue;
			}
